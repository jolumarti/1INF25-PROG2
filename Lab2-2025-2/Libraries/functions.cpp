...
void cargarStreamers(const char *file_name, char ***&streamers, int **&fechasPromedios,
    ...
    while (true) {
        ...
        file >> c >> tiempoRep[0] >> c >> fechaPromedio[2] >> c >> tiempoRep[1] >> c;
...
void allocate_inc_memory(int &cap, int &n, char ***&comentarios, char ***&etiquetas) {
    cap += INCREMENT;
...
void cargarComentarios(const char *file_name, char ***&comentarios, char ***&etiquetas) {
    ...
    int i = 0, cap = 0;
    while (true) {
        if (i == cap) allocate_inc_memory(cap, i, comentarios, etiquetas);
